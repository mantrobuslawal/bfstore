...
Define service identity for bfstore application services.
...
- Every service has a distinct runtime identity.
- Service identity is separate from human and CI/CD identity.
- AWS permissions are granted through workload identity, not shared node credentials.
