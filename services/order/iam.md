...
Define AWS IAM and runtime identity requirements for the `order` service.

## Scope

Applies to `order` deployment, Kubernetes service account, AWS workload role and secret access.
...
- Read order database secret.
- Call payment/shipping services through application paths.
