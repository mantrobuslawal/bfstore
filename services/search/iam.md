...
Define AWS IAM and runtime identity requirements for the `search` service.

## Scope

Applies to `search` deployment, Kubernetes service account, AWS workload role and secret access.
...
- Read search backend credentials.
- Consume indexing events.
- Potential object/storage access for snapshots later.
