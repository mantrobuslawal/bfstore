...
Define AWS IAM and runtime identity requirements for the `basket` service.

## Scope

Applies to `basket` deployment, Kubernetes service account, AWS workload role and secret access.
...
- Read basket database secret.
- Access cache credentials if cache is introduced.
