...
Define AWS IAM and runtime identity requirements for the `recommendation` service.

## Scope

Applies to `recommendation` deployment, Kubernetes service account, AWS workload role and secret access.
...
- Read recommendation data/model artefact credentials if introduced.
- No direct payment or secrets access beyond its own service.
