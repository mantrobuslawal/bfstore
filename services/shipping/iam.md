...
Define AWS IAM and runtime identity requirements for the `shipping` service.

## Scope

Applies to `shipping` deployment, Kubernetes service account, AWS workload role and secret access.
...
- Read shipping provider secrets if external provider is used.
- Publish shipment events.
- No access to payment secrets.
